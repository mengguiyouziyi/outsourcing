...
async def init(loop):
    ...
    await loop.create_server(app.make_handler(), '0.0.0.0', 8007)


if __name__ == '__main__':
    start = time.clock()
    # captchaTrain = CaptchaPredit("D:\\work\\shaohang\\py_spider\\py_spider\\get_code\\beijing")
    # captchaTrain = CaptchaPredit("D:\\work\\shaohang\\py_spider\\py_spider\\get_code\\shanghai")
    # captchaTrain = CaptchaPredit("D:\\work\\shaohang\\py_spider\\py_spider\\get_code\\susong")
    captchaTrain = CaptchaPredit(r"D:\MyPackage\MyProject\model\\")
    # captchaTrain = CaptchaPredit(r"/search/odin/meng/utility/models/cnn_captcha_model/")
    loop = asyncio.get_event_loop()
    loop.run_until_complete(init(loop))
    loop.run_forever()
"""
img_byte = base64.b64encode(response_img.content)
img_str = img_byte.decode('ascii')
data = {'img': img_str}
json_mod = json.dumps(data)
response_code = s.post(url='http://127.0.0.1:8008/zqsx', data=json_mod)
