    ...
    for file in os.listdir(filedir):
        if fnmatch(file, '*.jpg'):
            ...
            for i in range(cutting_img_num):
                try:
                    # file = '%s-cutting-%s.jpg' % (img_name.split('.')[0], i)
                    # file = os.path.join(os.getcwd(), 'out_img', file)
                    file = './out_img/%s-cutting-%s.jpg' % (img_name.split('.')[0], i)
                    print(file)
                    print(pytesseract.image_to_string(Image.open(file), lang='eng', config='--psm 10'))
                    # 识别验证码
                    str_img = str_img + pytesseract.image_to_string(Image.open(file), lang='eng',
